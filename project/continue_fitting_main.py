...
class TestBench():
    ...
    def train(self):
        for phase in range(2):
            print(f"Continue fitting phase:{phase}")
            ...
            mlp_loss = self.mlp.train(self.dataset[self.mlp.name][phase],
                            build_optimizer(self.config['optimizer'], self.mlp.model.parameters()),
                            iter=self.config['iterations'],
                            batch=self.config['batch_size'])
            
            kan_loss = self.kan.train(self.dataset[self.kan.name][phase], 
            ...
            plt.ylabel('Loss')
            plt.ylim(0, min(2.4, max(max(mlp_loss), max(kan_loss))))
            # plot both losses on the same graph with labels
            plt.plot(kan_loss, label='KAN')
            plt.plot(mlp_loss, label='MLP')
            plt.legend()
            # save the plot to a file
            plt.savefig(f"figs/{self.config['task']}/phase_{phase}_loss.png")
