...
def create_graph(img, mask, fg_prob, bg_prob):
    ...
    for y in range(img.shape[0]):
        for x in range(img.shape[1]):
            ...
            graph.add_edge('source', idx, capacity=fg_prob[y,x]+3*fg_dist[y,x]) # 100
            # elif mask[y, x] == 0:
            graph.add_edge(idx, 'sink', capacity=bg_prob[y,x]+3*bg_dist[y,x]) # 100
...
def segment_image(image, mask, gmm_model, round=10):
    img = image
    ...
    for _ in range(round):
        ...
        # fg_prob = np.exp(fg_prob)*1/256
        # bg_prob = np.exp(bg_prob)*1/256
        # import pdb; pdb.set_trace()
        fg_prob =(fg_prob - fg_prob.min()) / (fg_prob.max()-fg_prob.min())
        bg_prob =(bg_prob - bg_prob.min()) / (bg_prob.max()-bg_prob.min())
        fg_prob = -np.log(fg_prob + 1e-6)
        bg_prob = -np.log(bg_prob + 1e-6)
        # import pdb; pdb.set_trace()
...
def process(input, max_size=256):
    ...
    fg_gmm = GaussianMixture(n_components=1, random_state=0)
    bg_gmm = GaussianMixture(n_components=1, random_state=0)
